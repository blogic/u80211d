...
static bool
nl_raw_socket(struct nl_socket *ev, int protocol, int groups,
...
bool
nl_socket(struct nl_socket *ev, int protocol,
		int (*cb)(struct nl_msg *msg, void *arg), void *priv)
{
	if (!nl_raw_socket(ev, protocol, 0, nl_handler_nl_status, ULOOP_ERROR_CB))
		return false;
